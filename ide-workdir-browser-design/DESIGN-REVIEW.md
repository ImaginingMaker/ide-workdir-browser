...
图标预设均已有对应状态稿；关于页已补充当前版本、手动更新检查和新版本下载状态。
...
| 设置       | 设置页为独立工作台，不显示浏览工具栏/标签/路径栏/状态栏；关于页提供手动稳定版检查和安全下载状态，不表现自动安装                 |
| 响应式     | 窄窗侧栏折叠、Inspector 关闭、最小窗口和暗色状态均已覆盖                                                                        |

## 2026-08-07 更新检查对齐

- B23 已从过期的 `v1.0.0` 修正为当前 `0.1.0` 版本口径。
- 身份卡内新增“检查更新”、发现新版本和“查看并下载”状态，复用 primary、success 和 secondary
  语义色。
- 真实 React 仅在用户点击后检查稳定版；设计稿不展示启动检查、beta 通道、后台下载或自动安装。
- 43 个源状态已重新生成并合并为 7 个分类画布，7 个根画布执行 `design:refine` 均为 0 项修复。
